"""
Build genraton_apps.json by re-scraping each page and parsing immediately.
This script saves markdown to files and parses them.
"""
import sys
import re
import json
...
for page_file in page_files:
    with open(page_file, 'r', encoding='utf-8') as f:
        md = f.read()
    print(f'\nProcessing {page_file} ({len(md)} chars)...')
    apps = parse_markdown_for_cards(md)
    print(f'  Parsed {len(apps)} cards')
    for app in apps:
...
print(f'\n{"="*60}')
print(f'Total unique cards: {len(all_apps)}')

# Build output in the format import_genraton.py expects
output = {
    'installed_apps': [
        {
            'app': {
