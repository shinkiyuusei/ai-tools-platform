"""
One-shot script: scrape all genraton pages via Firecrawl and generate genraton_apps.json.
Uses Firecrawl's API through the MCP responses saved in this session.
"""
import sys, re, json, io, os

sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')

# The markdown content from pages 1-7 scrapes are saved in the tool-results directory
# We'll find them and parse them all

BASE_DIR = r'C:\Users\40935\.claude\projects\c--Users-40935-Desktop-python-ai-tools-platform\347ab3dc-545b-4711-abb6-2d153af75822\tool-results'

# Find all firecrawl scrape result files
import glob
result_files = sorted(glob.glob(os.path.join(BASE_DIR, 'mcp-firecrawl-firecrawl_scrape-*.txt')))
print(f'Found {len(result_files)} Firecrawl scrape result files')

SEP = chr(92) + chr(92) + chr(10) + chr(92) + chr(92) + chr(10)
LINE_END = chr(92) + chr(92) + chr(10)

def parse_markdown_for_cards(md):
    apps = []
    seen_ids = set()
    for author_match in re.finditer(r'作者[：:]', md):
        author_pos = author_match.start()
        after_text = md[author_pos:]
        link_match = re.search(r'\]\(https://genraton\.xyz/explore/installed/([a-f0-9-]+)\)', after_text)
        if not link_match:
            continue
        ...
        remaining = clean_parts[1:] if (img_match and img_match.group(0) == clean_parts[0].strip()) else clean_parts
        if len(remaining) < 3:
            continue
...
for fpath in result_files:
    try:
        ...
        if 'explore/installed/' not in md:
            continue  # skip non-list pages

        apps = parse_markdown_for_cards(md)
        fname = os.path.basename(fpath)
        print(f'{fname}: {len(apps)} cards')
...
# Also check saved page files
for pf in sorted(glob.glob('genraton_page*.md')):
    ...
    for app in apps:
        if app['name'] not in seen_names:
            seen_names.add(app['name'])
            all_apps.append(app)
    print(f'{pf}: {len(apps)} cards (total unique now: {len(all_apps)})')

print(f'\nFinal total: {len(all_apps)} unique cards')

# Sort by rating
all_apps.sort(key=lambda a: float(a['rating']) if a['rating'].replace('.','').isdigit() else 0, reverse=True)
