...
def get_analytics_service():
    ...
    return AnalyticsService(get_mongo_db(), get_redis_client())
...
@analytics_bp.get("/analytics/tool/<int:tool_id>")
def get_tool_analytics(tool_id: int):
    """Get analytics data for a specific tool"""
    days = min(int(request.args.get("days", 30)), 90)
    
    service = get_analytics_service()
    analytics = service.get_tool_analytics(tool_id, days)
